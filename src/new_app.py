...
from components.enhanced_document_processor import EnhancedDocumentProcessor
from components.enhanced_vector_store import EnhancedVectorStore
from components.advanced_llm_handler import AdvancedLLMHandler
from components.enhanced_rag_pipeline import EnhancedRAGPipeline
...
def display_chat_message(
    ...
    """Display a chat message with optional sources and debug info."""
    with st.chat_message(role):
        st.markdown(content)

        # Display sources only for assistant replies
                    ...
                    if source.get("metadata"):
                        ...
                        with cols[2]:
                            if metadata.get("entities"):
                                ...
                                st.markdown(", ".join(metadata["entities"][:3]))
                        with cols[3]:
                            st.markdown("**🔗 Source**")
        ...
        if debug_info and st.session_state.get("debug_mode") and role == "assistant":
...
def main():
    ...
    with st.sidebar:
        ...
        if st.button("🔄 Reset System"):
            for key in ['messages', 'vector_store', 'rag_pipeline', 'document_processed', 'num_chunks', 'processing_stats']:
